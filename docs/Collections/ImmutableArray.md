...
To create a new immutable array, use `ImmutableArray(Values*)` or `Tuple(Values*)`. Use `ImmutableArray.FromArray(A)` or `IArray#Freeze()` to create a read-only view of an existing array.

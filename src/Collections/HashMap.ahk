 ...
 * 
 * Instead of relying on object identitity as keys, `HashMap` is able to
 * compare keys with user-defined `.Eq()` methods.
 * 
 * Elements are distributed into buckets using a hash code generated by
 * calling `.HashCode()` on the value.
 * 
 * ---
 * 
 * Keys must implement proper {@link AquaHotkey_Eq `.Eq()`} and
 * {@link AquaHotkey_Hash `.HashCode()`}. In relation to hash tables,
 * more importantly:
 * 
 * 1. If `A.Eq(B)`, then `A.HashCode() == B.HashCode()`.
 * 2. Both `.Eq()` and `.HashCode()` should use the same fields or
 *    characteristics of a value in order to detect equality and creating
 *    a hash code, respectively.
 * 
 * ---
 * 
